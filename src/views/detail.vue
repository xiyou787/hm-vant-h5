...
<script>
import { detail, updateLikeColl } from '@/api/article'
...
export default {
  ...
  methods: {
    async toggleLike () {
      await updateLikeColl(this.article.id, 1)
      ...
      if (this.article.likeFlag) {
        ...
        this.article.likeCount--
        this.$toast.success('点赞取消')
      }
    },
    async toggleCollect () {
      await updateLikeColl(this.article.id, 2)
      this.article.collectFlag = !this.article.collectFlag
      if (this.article.collectFlag) {
        this.$toast.success('收藏成功')
      } else {
        this.$toast.success('收藏取消')
      }
    }
  }
}
